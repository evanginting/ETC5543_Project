...
#### Prediction vs Actual Comparison
```{r viz_comparison3_hree, eval = FALSE, include = FALSE, warning = FALSE, error = FALSE}
results_hree_test %>%
  mutate(train = "testing") %>%
  bind_rows(results_hree_train %>%
              mutate(train = "training")) %>%
...
```{r}
# Define the recipe for XGBoost
xgb_recipe_hree <- recipe(HREE ~ ., data = DT_hree_train_fltr_3) 
...
xgboost_params_hree <- parameters(
  ...
  min_n(),
  sample_size = sample_prop(),
  finalize(mtry(), DT_hree_train_fltr_3))
...
xgb_tune_hree <- xgb_tuned_workflow_hree %>% 
  tune_grid(resamples = DT_hree_fltr_folds_3, 
...
# Make predictions on the test set
xgb_hree_train_predictions <- predict(xgb_tuned_fit_hree, new_data = DT_hree_train_fltr_3)

# Make predictions on the test set
xgb_hree_test_predictions <- predict(xgb_tuned_fit_hree, new_data = DT_hree_test_fltr_3)
```
...
results_hree_test_best <- results_hree_test %>%
  filter(model %in% c("RF_Rdm_Tuned_HREE_3")) %>%
  ggplot(aes(.pred, truth)) +
  geom_abline(lty = 2, linewidth = 0.75, linetype = "dashed", colour='red') +
  geom_point(alpha = 0.7, colour = "black", size = 5) +
  xlim(c(0,35)) +
