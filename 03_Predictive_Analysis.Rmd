...
### Random Forest Model: Option 5 (All elements from ME-4ACD81) - NO LONGER USED
...
#### Build the Model - NO LONGER USED
```{r, include = FALSE, eval = FALSE}
# Create a recipe for preprocessing
...
#### Evaluate the Model - NO LONGER USED
```{r, include = FALSE, eval = FALSE}
# RF Tuned Results for all elements from ME-4ACD81
## Training set evaluation
...
#### Prediction vs Actual Comparison - NO LONGER USED
```{r viz_comparison5, warning = FALSE, error = FALSE, include = FALSE, eval = FALSE}
results_test %>%
  mutate(train = "testing") %>%
  bind_rows(results_train %>%
              mutate(train = "training")) %>%
...
# Saving the data into .RDS format for final report & presentation purpose
saveRDS(results_test, './Final_report/Final_report_files/ree_results_test.rds')
...
results_ree_test_df <- results_test %>%
  ...
  summarise(RMSE = RMSE(.pred, truth),
            ...
            MAE = MAE(.pred, truth),
            MAPE = MAPE(.pred, truth)) %>%
  filter(model %in% c("RF_Rdm_Tuned_2","RF_Rdm_Tuned_3", "RF_Rdm_Tuned_4", "XGBoost_Tuned")) %>%
...
```{r ree_pred_res, fig.cap = "Actual vs Prediction Results", fig.width = 8, fig.height = 8, fig.align = "center", warning = FALSE, error = FALSE}
...
results_test_best <- results_test %>%
  ...
  annotate("text", x = 390, y = 76, label = paste("RMSE: ", round(ree_rmse_value, 2)), size = 9, hjust = 1) +
  annotate("text", x = 390, y = 51, label = paste("MAE: ", round(ree_mae_value, 2)), size = 9, hjust = 1) +
  annotate("text", x = 390, y = 26, label = paste("MAPE: ", round(ree_mape_value, 2)), size = 9, hjust = 1) +
  annotate("text", x = 390, y = 1, label = paste("R-squared: ", round(ree_rsq_value, 2)), size = 9, hjust = 1)
...
#### Prediction vs Actual Comparison
```{r viz_comparison1_hree, warning = FALSE, error = FALSE}
results_hree_test %>%
  mutate(train = "testing") %>%
...
### Random Forest Model: Option 2 (All elements from ME-4ACD81) - NO LONGER USED
...
#### Prediction vs Actual Comparison - NO LONGER USED
```{r viz_comparison2_hree, eval = FALSE, include = FALSE, warning = FALSE, error = FALSE}
results_hree_test %>%
  mutate(train = "testing") %>%
  bind_rows(results_hree_train %>%
              mutate(train = "training")) %>%
...
```{r}
# Filter the training and test datasets to include predictors and target variables only
DT_hree_train_fltr_3 <- DT_hree_train[,c("HREE","Pb", "Sc")] 
DT_hree_test_fltr_3 <- DT_hree_test[,c("HREE","Pb", "Sc")]
...
```{r}
# Define the recipe for XGBoost
xgb_recipe_hree <- recipe(HREE ~ ., data = DT_hree_train_fltr) 
...
xgboost_params_hree <- parameters(
  ...
  min_n(),
  sample_size = sample_prop(),
  finalize(mtry(), DT_hree_train_fltr))
...
xgb_tune_hree <- xgb_tuned_workflow_hree %>% 
  tune_grid(resamples = DT_hree_fltr_folds, 
...
# Make predictions on the test set
xgb_hree_train_predictions <- predict(xgb_tuned_fit_hree, new_data = DT_hree_train_fltr)

# Make predictions on the test set
xgb_hree_test_predictions <- predict(xgb_tuned_fit_hree, new_data = DT_hree_test_fltr)
```
...
# Saving the data into .RDS format for final report & presentation purpose
saveRDS(results_hree_test, './Final_report/Final_report_files/hree_results_test.rds')
...
#### Prediction vs Actual Comparison
```{r viz_comparison_xgb_hree, warning = FALSE, error = FALSE}
results_hree_test %>%
  mutate(train = "testing") %>%
  bind_rows(results_hree_train %>%
              mutate(train = "training")) %>%
...
results_hree_test_df <- results_hree_test %>%
  ...
  mutate(model_scenarios = case_when(model == "RF_Rdm_Tuned_HREE" ~ "Default tuned",
                                    ...
                                    model == "XGBoost_Tuned_HREE" ~ "Model with high variable importance")) %>%
  mutate(elements = case_when(model == "RF_Rdm_Tuned_HREE_3" ~ "Pb, Sc",
                                    ...
                                    model == "XGBoost_Tuned_HREE" ~ "Pb, Sc")) %>%
...
# Getting metrics values
hree_rmse_value <- results_hree_test_df[1,5]
hree_rsq_value <- results_hree_test_df[1,8]
hree_mae_value <- results_hree_test_df[1,6]
hree_mape_value <- results_hree_test_df[1,7]
...
results_hree_test_best <- results_hree_test %>%
  ...
  theme(axis.title.x = element_text(size = 24),            # Increase X-axis title size
        axis.title.y = element_text(size = 24)) +          # Increase Y-axis title size
  # Annotate the plot with the metrics
  annotate("text", x = 29, y = 7, label = paste("RMSE: ", round(hree_rmse_value, 2)), size = 9, hjust = 1) +
  annotate("text", x = 29, y = 5, label = paste("MAE: ", round(hree_mae_value, 2)), size = 9, hjust = 1) +
  annotate("text", x = 29, y = 3, label = paste("MAPE: ", round(hree_mape_value, 2)), size = 9, hjust = 1) +
  annotate("text", x = 29, y = 1, label = paste("R-squared: ", round(hree_rsq_value, 2)), size = 9, hjust = 1)
...
#### Prediction vs Actual Comparison
```{r viz_comparison_lree, warning = FALSE, error = FALSE}
results_lree_test %>%
  mutate(train = "testing") %>%
...
#### Prediction vs Actual Comparison - NO LONGER USED
```{r viz_comparison2_lree, warning = FALSE, error = FALSE, include = FALSE, eval = FALSE}
results_lree_test %>%
  mutate(train = "testing") %>%
  bind_rows(results_hree_train %>%
              mutate(train = "training")) %>%
...
#### Prediction vs Actual Comparison
```{r viz_comparison3_lree, warning = FALSE, error = FALSE}
results_lree_test %>%
  mutate(train = "testing") %>%
  bind_rows(results_lree_train %>%
              mutate(train = "training")) %>%
...
#### Prediction vs Actual Comparison
```{r viz_comparison4_lree, warning = FALSE, error = FALSE}
results_lree_test %>%
  mutate(train = "testing") %>%
  bind_rows(results_lree_train %>%
              mutate(train = "training")) %>%
...
# Saving the data into .RDS format for final report & presentation purpose
saveRDS(results_lree_test, './Final_report/Final_report_files/lree_results_test.rds')
...
#### Prediction vs Actual Comparison
```{r viz_comparison_xgb_lree, warning = FALSE, error = FALSE}
results_lree_test %>%
  mutate(train = "testing") %>%
  bind_rows(results_lree_train %>%
              mutate(train = "training")) %>%
...
results_lree_test_df <- results_lree_test %>%
  ...
  summarise(RMSE = RMSE(.pred, truth),
            ...
            MAE = MAE(.pred, truth),
            MAPE = MAPE(.pred, truth)) %>%
  filter(model %in% c("RF_Rdm_Tuned_LREE","RF_Rdm_Tuned_LREE_3","RF_Rdm_Tuned_LREE_4", "XGBoost_Tuned_LREE")) %>%
  ...
  mutate(model_scenarios = case_when(model == "RF_Rdm_Tuned_LREE" ~ "Default tuned",
                                    ...
                                    model == "XGBoost_Tuned_LREE" ~ "Elements with correlation coefficient above 0.5")) %>%
  mutate(elements = case_when(model == "RF_Rdm_Tuned_LREE" ~ "All elements",
                                    ...
                                    model == "XGBoost_Tuned_LREE" ~ "Pb, Sc, Zn")) %>%
...
# Getting metrics values
lree_rmse_value <- results_lree_test_df[3,5]
lree_rsq_value <- results_lree_test_df[3,8]
lree_mae_value <- results_lree_test_df[3,6]
lree_mape_value <- results_lree_test_df[3,7]
...
results_lree_test_best <- results_lree_test %>%
  ...
  theme(axis.title.x = element_text(size = 24),            # Increase X-axis title size
        axis.title.y = element_text(size = 24)) +          # Increase Y-axis title size
  # Annotate the plot with the metrics
  annotate("text", x = 290, y = 61, label = paste("RMSE: ", round(lree_rmse_value, 2)), size = 9, hjust = 1) +
  annotate("text", x = 290, y = 41, label = paste("MAE: ", round(lree_mae_value, 2)), size = 9, hjust = 1) +
  annotate("text", x = 290, y = 21, label = paste("MAPE: ", round(lree_mape_value, 2)), size = 9, hjust = 1) +
  annotate("text", x = 290, y = 1, label = paste("R-squared: ", round(lree_rsq_value, 2)), size = 9, hjust = 1)
