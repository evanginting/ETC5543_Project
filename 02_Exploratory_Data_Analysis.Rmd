...
The table \@ref(tab:dat-preview) provide a preview of the data from first ten observations.

```{r dat-preview, fig.cap = "First ten observations of the data"}
kable(head(alldata, n = 10),
      col.names = gsub("[_]", " ", names(alldata)),
      align = "llcccccl",
      caption = "First five observations of the data",
      format = "html") %>%
...
desc_stats <- alldata %>%
  ...
  summarise(min = min(PAAS_normalised_value),
         ...
         range = max(PAAS_normalised_value) - min(PAAS_normalised_value),
         q1 = quantile(PAAS_normalised_value, 0.25),
         q3 = quantile(PAAS_normalised_value, 0.75),
         iqr = IQR(PAAS_normalised_value),
         sd = sd(PAAS_normalised_value),
         var = var(PAAS_normalised_value),
         skewness = moments::skewness(PAAS_normalised_value),
...
We are going to analyse the distribution of each critical elements based on their PAAS normalised values. The figure \@ref(fig:element-dist) provide insights into the spread, central tendency, and potential outliers for each element.
...
As depicted, there are some key points that we would like to raise, which are:
...
- **Outliers**: Several elements have significant outliers, as indicated by the dots outside the whiskers of the box plots. For instance, **Ge**, **Bi**, **Mn**, and **Ba** show notable outliers far from the main data range. These outliers suggest the presence of some unusually high or low values for these elements, which could be of interest for further investigation.
...
- **Comparison Accross Elements**: **Ge** stands out with a particularly large spread and median, making it an outlier among the elements. Conversely, many rare earth elements (REE, REEY, MREE, LREE, HREE) have relatively low medians and a small spread, indicating that their normalized values are generally low and clustered.
...
```
As can be seen, the majority of normalised value fall within 0 to 10, while the rest are can be counted as outliers. Some highlight points from this plot are:

- **Ag** does not have any normalised PAAS values below background, indicating that coal waste is rich in 'Ag'. 
- **Re**, **Ge** have significant concentration difference, which make their samples fall in both categories.
- **HREE** concentration is always below background value.
- All samples that categorised as "below background" are actually just fall close to the threshold (between 0 and 1).

Furthermore, we also analyse how the distribution of the PAAS levels across various Project Area. The figure \@ref(fig:figprojectcons) shows how many elements that was recorded in each project area that fall into these two categories.
...
Lastly, let's see how is the distribution of each critical element on every project area. The figure \@ref(fig:element-pro-dist) below provide this information. As the continuation of the previous analysis, in this part we will focus more on the concentration of each element in each project area. For instance, such insights that we are going to look for are elements that has wide range of concentrations in one project, elements that have above/below background in the same project area.
...
- **Collinsville**: **Co** has wide range of concentration, which make some samples fall below background, while there is one sample that has high concentration, indicating that in one project area the same element can be significantly different. - **Confidential_A**: the outlier of **Tl** that was seen in figure \@ref(fig:element-dist) and figure \@ref(fig:figelementcons) is found here, suggesting high concentration of this element in this project area. 
- **Confidential_B**: Outliers of multiple elements (**Mo**, **Li**, **Cr**, **Ba**) that were reported in figure \@ref(fig:element-dist) are spotted in this project area. This fact is relevant with the figure \@ref(figprojectcons) where this project area has many samples taken from. Additionally, we can spot a wide range of **Ba** concentration, where one of the sample is categorised as "below background", while there are two samples that categorised as "above background" with very high normalised value (above 10).
...
```{r corplot, fig.cap = "Correlation Matrix Plot of Critical Elements", out.width = "90%", fig.width = 6, fig.height = 6, fig.align = "center"}
res_cor_cop <- alldata %>% 
...
In the next section, we are going to dig deeper into those correlated elements with value above 0.95. But before, those elements are known as lanthanides series. Lanthanides are a group of the first 15 f-block elements with atomic numbers from 57 to 71. In addition to yttrium, which share many similar chemical properties with the lanthanides, these elements comprise the rare earth elements (REEs) (Mattocks et al., 2021). 

Seredin (2012) suggest that the first suggestions for recovering lanthanides and yttrium (REY) as by-products from coal deposits can be traced back to 20 years ago, following the discovery of coal beds in a Russian Far East (RFE) basin that had high REY content (0.2% - 0.3%). Additional coal seams with comparable or even higher REY concentrations (up to 1.0% in ash) were identified in six coal-bearing basins across the same region. Since then, REY-rich coal has also been discovered in coal basins in various other countries. Thus, the lanthanides are found in coal and coal by-products could be possible because of their association with the materials that make up coal. 
