...
The table \@ref(tab:dat-preview) provide a preview of the data from first five observations.

```{r dat-preview, fig.cap = "First five observations of the data"}
kable(head(alldata, n = 5),
...
```
As can be seen, the majority of normalised value fall within 0 to 10, while the rest are can be counted as outliers. A highlight point from this plot is 'Ag' does not have any normalised PAAS values below standard, indicating that coal waste is rich in 'Ag'. 
...
We are going to dig deeper into those correlated elements with value above 0.95 in the next section. But before, those elements are known as lanthanides series. Lanthanides are a group of the first 15 f-block elements with atomic numbers from 57 to 71. In addition to yttrium, which share many similar chemical properties with the lanthanides, these elements comprise the rare earth elements (REEs) (...). 

... suggest that the first suggestions for recovering lanthanides and yttrium (REY) as by-products from coal deposits can be traced back to 20 years ago, following the discovery of coal beds in a Russian Far East (RFE) basin that had high REY content (0.2% - 0.3%). Additional coal seams with comparable or even higher REY concentrations (up to 1.0% in ash) were identified in six coal-bearing basins across the same region. Since then, REY-rich coal has also been discovered in coal basins in various other countries. Thus, the lanthanides are found in coal and coal by-products could be possible because of their association with the materials that make up coal. 
...
grid.arrange(combined_plots, legend, ncol = 2, heights = c(10, 1))
```
