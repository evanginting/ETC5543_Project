...
```{r figprojectcons2, fig.cap = "The Profile of Project Area with above Background Level", out.width = "80%", fig.width = 10, fig.height = 8, fig.align = "center"}
alldata %>%
  group_by(Project_Name, Above_PASS_flag) %>%
  filter(Above_PASS_flag == "Enriched above background") %>%
  summarise(count = n()) %>%
  #mutate(Project_Name = reorder(Project_Name, count, sum)) %>%  # Reorder based on the sum of counts
  ggplot(aes(x = count, y = reorder(Project_Name, count), fill = Above_PASS_flag)) +
  geom_text(aes(label = count), size = 3, vjust = 0.5, hjust = -0.1, color = "black", position = position_dodge(0.9)) +
  geom_col(position = "dodge") +
  #facet_wrap(~ Project_Name) +
  theme(axis.text.x = element_blank()) +
