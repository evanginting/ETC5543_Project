...
ree_calc_all <- ree_all %>% 
  mutate(REE = rowSums(select(., La, Ce, Pr, Nd, Sm, Eu, Gd, Tb, Dy, Ho, Er, Tm, Yb, Lu), na.rm = TRUE)) %>% 
  mutate(REEY = rowSums(select(., La, Ce, Pr, Nd, Sm, Eu, Gd, Tb, Dy, Ho, Er, Tm, Yb, Lu, Y), na.rm = TRUE)) %>% 
  mutate(LREE = rowSums(select(., La, Ce, Pr, Nd, Sm), na.rm = TRUE)) %>%  # note Pm not tested
  mutate(MREE = rowSums(select(., Eu, Gd, Tb, Dy, Y), na.rm = TRUE)) %>% 
  mutate(HREE = rowSums(select(., Ho, Er, Tm, Yb, Lu), na.rm = TRUE)) 
