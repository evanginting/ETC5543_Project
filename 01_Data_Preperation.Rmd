...
BR22046077_data_reformatted <- BR22046077_raw %>%
  ...
  mutate(across(everything(), ~ ifelse(grepl("<", .), -999, .))) %>%
  mutate(across(-Sample, ~ round(as.numeric(.), 4))) %>%
...
```{r}
# 32 elements are having multiple concentration result (ME-MS81 & ME-MS61r). 
...
# BR23093631_raw <- read.csv("./data/BR23093631.csv",skip=9, col.names = names(read.csv("./data/BR23093631.csv", skip=8))) 
...
BR23093631_raw <- read.csv("./data/BR23093631.csv", header = TRUE, skip=9) 

colnames(BR23093631_raw) <- paste(header1, header2, sep = "_")  # Combine the two headers with an underscore
colnames(BR23093631_raw)[1] <- "Sample"  # Fix the first column name if necessary
...
BR23093631_data_reformatted <- BR23093631_raw %>%
  ...
  mutate(Symbol = ifelse(grepl("_", str_sub(sym_method,start=-2)), str_sub(sym_method,start=-1), str_sub(sym_method,start=-2))) %>%
  inner_join(select(for_filter, sym_method), by="sym_method") %>%
  na.omit() %>% #remove missing data
  mutate(Project_Area = "Confidential") %>% #add project area name
...
```{r}
all_data_bfr_paas <- bind_rows(C29030_data_reformatted, BR22046077_data_reformatted, ws_ree_data_reformatted)
