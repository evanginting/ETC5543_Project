...
CE_list|>kbl( align = "c",
              escape=FALSE,
               booktabs = T,
              caption = "<b>Summary of Critical Elements: Production, Global Share, Projected Demand, and Usage</b>")|>
...
```{r concentration}
table2_c<-tibble("Critical element"=c("Lithium","REE","Cobalt","Nickel","Tantalum","Vanadium","Zirconium","Gallium","Bismuth","Chromium","Niobium","Molybdenum"),
       ...
       "Higest concentration(ppm)"=c(25,205,30,40,1,140,160,25,1,51,32,3),
                                     
       "% of above crustal average"=c(2/9,1/9,4/9,0,1/3,2/9,0,4/9,2/3,0,1/9,7/9)
       )



table2_sm<-tibble(
  "Critical element"=c("Lithium","REE","Cobalt","Nickel","Tantalum","Vanadium","Zirconium","Gallium","Bismuth","Chromium","Niobium","Molybdenum"),
       "PAAS standard"=c(21,184,17,47,1,97,193, 17,0.2,92,12,1),
  "Average concentration(ppm)"= c(17.2,138.8,39.3,25.8,0.2,102,116.8,15,0.18,51.33,4.67,0.67),
       "Higest concentration(ppm)"=c(28,189,134,73,1,225,243,22,0.4,266,8,1),
                                     
       "% of above crustal average"=c(1/3,1/6,2/3,1/3,1/6,1/3,1/6,1/3,2/3,1/6,0,2/3)
)
...
table2_tf<-tibble(
  ...
  "Average concentration(ppm)"= c(21.5,244,9.25,3.75,1.1,27.9,153.75,32.25,0.71,0,9.63,1),
       "Higest concentration(ppm)"=c(105,441,19,30,2,70,282,37,1.2,0,18,5),
                                     
       "% of above crustal average"=c(1/8,7/8,1/8,0,1,0,1/4,1,1,0,1/4,1/2)
)

table2_bind<-rbind(table2_c,table2_sm,table2_tf)

table2_bind<-table2_bind|>
  mutate(`Average concentration(ppm)` = cell_spec(`Average concentration(ppm)`, 
                                                  "html",
                                                  bold = ifelse(`Average concentration(ppm)` > `PAAS standard`, TRUE, FALSE)),
  `Higest concentration(ppm)`= cell_spec(`Higest concentration(ppm)`, 
                                                  "html",
                                                  bold = ifelse(`Higest concentration(ppm)` > `PAAS standard`, TRUE, FALSE)),
  `% of above crustal average`=paste0(round(`% of above crustal average`*100),"%"))
table2_bind|>kbl( align = "c",
              escape=FALSE,
               booktabs = T,
              caption = "<b>Summary of Hodginkson's research on critical element mapping in coal mines</b>")|>

  kable_classic_2(full_width = F)|> 
  pack_rows("Coal seam& associate",1,12)|>
pack_rows("Siltstone & mudstone",13,24) |>
  pack_rows("Tuffaceous rocks",25,36) |>
  footnote( general = " Hodginkson et al., 2020",
